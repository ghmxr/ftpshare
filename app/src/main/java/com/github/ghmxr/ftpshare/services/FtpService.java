    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        try{
           FtpServerFactory factory=new FtpServerFactory();
            ...
            baseUser.setHomeDirectory("/storage/emulated/0/backup");

            BaseUser baseUser2 = new BaseUser();
            baseUser2.setName("admin");
            baseUser2.setPassword("admin");
            baseUser2.setHomeDirectory("/storage/emulated/0/");

            List<Authority> authorities = new ArrayList<Authority>();
            authorities.add(new WritePermission());

            baseUser2.setAuthorities(authorities);
            factory.getUserManager().save(baseUser);
            factory.getUserManager().save(baseUser2);
